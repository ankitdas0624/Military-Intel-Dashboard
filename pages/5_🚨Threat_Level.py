...
encoders, target_encoder = load_encoders()

countries = list(encoders["country_txt"].classes_)
regions = list(encoders["region_txt"].classes_)

attacks = (
    list(encoders["attacktype1_txt"].classes_)
    if "attacktype1_txt" in encoders
    else sorted(df["attacktype1_txt"].dropna().unique())
)
...
selected_weapon = st.sidebar.selectbox(
    "Weapon Type",
    weapons
)
...
try:
    encoded_country = encoders["country_txt"].transform(
        [selected_country]
    )[0]

    encoded_region = encoders["region_txt"].transform(
        [selected_region]
    )[0]

    if "attacktype1_txt" in encoders:
        encoded_attack = encoders["attacktype1_txt"].transform(
            [selected_attack]
        )[0]
    else:
        encoded_attack = 0

    encoded_weapon = encoders["weaptype1_txt"].transform(
        [selected_weapon]
    )[0]

    encoded_target = encoders["targtype1_txt"].transform(
        [selected_target]
    )[0]
...
col1.metric(
    selected_country,
    f"Code: {encoded_country}"
)

col2.metric(
    selected_region,
    f"Code: {encoded_region}"
)

col3.metric(
    selected_attack,
    f"Code: {encoded_attack}"
)

col4.metric(
    selected_weapon,
    f"Code: {encoded_weapon}"
)

col5.metric(
    selected_target,
    f"Code: {encoded_target}"
)
...
if st.button("🚨 Predict Threat Level", width="stretch"):
    ...
    if avg_fatalities >= 5 or total_incidents > 500:
        ...
        alert_type = st.error
        desc = (
            "High severity threat profile detected based on "
            "historical casualties and incident frequency."
        )
        ...
        desc = (
            "Moderate threat profile. Elevated vigilance recommended."
        )
        ...
        desc = (
            "Low historical risk profile for selected parameters."
        )

    alert_type(
        f"### Assessed Threat Level: {threat_level}"
    )
